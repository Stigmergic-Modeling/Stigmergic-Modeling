define("js/app/workspace", [ "../lib/jquery", "../lib/bootstrap", "../lib/mousewheel", "../lib/typeahead.jquery", "../lib/objectid", "../module/model", "../module/modelview", "../lib/d3", "../module/mousedrag" ], function(require, exports, module) {
    /*  -------  *
     *  载入模块
     *  -------  */
    // 通用库模块
    ...
    require("../lib/mousewheel")($);
    // jQuery 鼠标滚轮事件插件
    require("../lib/typeahead.jquery")($);
    // jQuery 输入框下拉提示插件
    var ObjectId = require("../lib/objectid");
    // 用于在浏览器端生成 Mongodb 的 ObjectId
    // 内部模块
    var Model = require("../module/model");
    var modelView = require("../module/modelview");
    // modelView 是一个函数，接受 icm 作为参数
    // 调试模块
     ...
     *  ---------  */
    // ICM 前端模型
    var icm = new Model(dataPassedIn.model);
    // dataPassedIn 通过后端的 .ejs 模板传入
    //var icm = new Model(debug.model);  // 调试时使用，从 debug 模块获取 model 数据
    //console.log('dataPassedIn.model', dataPassedIn.model);
    //console.log('icm', icm);
    // 记录 workspace 页面的状态
    var stateOfPage = {
        user: dataPassedIn.user,
        // dataPassedIn 通过后端的 .ejs 模板传入
        modelID: dataPassedIn.modelID,
        // dataPassedIn 通过后端的 .ejs 模板传入
        modelName: dataPassedIn.modelName,
        // dataPassedIn 通过后端的 .ejs 模板传入
        flagCRG: 0,
        // 0: class, 1: relationGroup
        flagDepth: 0,
        // for class: (0: class, 1: attribute, 2: propertyOfA)
    ...
    function controlPage() {
        ...
        // 一切“编辑”按钮的点击编辑功能
        $(document).on("click", ".stigmod-clickedit-btn-edit", handleEnterEdit);
        // “编辑”按钮的点击编辑功能
        $(document).on("click", ".stigmod-clickedit-btn-ok", handleSubmitEdit);
        // 编辑组件内的“提交”按钮功能
        $(document).on("click", ".stigmod-clickedit-btn-cancel", handleCancelEdit);
        // 编辑组件内的“取消”按钮功能
         ...
         * @returns {Function}
         */
        var substringMatcher = function(flag, maxLength) {
            // 所生成的 matches 集合的最大长度（对于每一个 dataset 来说）
            return function findMatches(q, cb) {
                var matches, substrRegex, strs = flag === "class" ? Object.keys(icm[0]) : Object.keys(icm[1]);
                // 将 strs 的生成写在 findMatches 函数中，可保证每次查询时 icm 都是最新的
    ...
    function modifyLeft(model, name) {
        ...
        fillLeft(model);
        // 重新激活
        var $this = $(document).find("#stigmod-nav-left-scroll .panel .list-group span[stigmod-nav-left-tag=" + name + "]").parent();
    ...
    function insertMiddle(model, name, noUnfold) {
        ...
        // 计算新 .panel 的编号
        var strPanel = "#stigmod-cont-right .panel " + (0 === stateOfPage.flagCRG ? ".stigmod-attr-cont-middle-title" : ".stigmod-rel-cont-middle-title");
        var $panelTitle = $(strPanel);
        // 取出所有 .panel
        ...
        if ("@" === stateOfPage.addAttrRel.position) {
            ...
            if (0 === stateOfPage.addAttrRel.direction) {
                // 上插
                $compo = $("#stigmod-cont-right .panel[stigmod-attrel-name=" + stateOfPage.addAttrRel.position + "]").before(0 === stateOfPage.flagCRG ? componentMiddleAttribute : componentMiddleRelation).prev();
            } else {
                // 下插
                $compo = $("#stigmod-cont-right .panel[stigmod-attrel-name=" + stateOfPage.addAttrRel.position + "]").after(0 === stateOfPage.flagCRG ? componentMiddleAttribute : componentMiddleRelation).next();
            }
        }
        ...
        var $collapseTrigger = $compo.find(strTitle).attr({
    ...
    function fillLeft(model) {
        ...
        $compo = $("#stigmod-nav-left-scroll .panel:first-child .list-group").empty();
        // 清空
        var modelClasses = model[0];
        for (var modelClass in modelClasses) {
            // 类名读入数组
        ...
        // 排序
        for (i = 0; i < modelClassOrdered.length; ++i) {
            // 类名
        ...
        $compo = $("#stigmod-nav-left-scroll .panel:last-child .list-group").empty();
        // 清空
        for (var modelRelationGroup in modelRelationGroups) {
            // 关系组名读入数组
        ...
        // 排序
        for (i = 0; i < modelRelationGroupOrdered.length; ++i) {
            // 关系组名
    ...
    function fillMiddle(model) {
        ...
        for (var i = 0; i < modelAttribute.length; ++i) {
    ...
    function getInputCheckResult(model, inputCase, input) {
        ...
        switch (inputCase) {
          // 类名
            ...
            } else if (model.doesNodeExist(0, input)) {
                // 类名重复
          ...
          case "class-modify":
            ...
            if (!pattern.test(input)) {
                // 格式不合法
                return "Valid Format: " + pattern.toString();
            } else if (stateOfPage.class !== input && model.doesNodeExist(0, input)) {
          ...
          // 关系组
            ...
            if (!model.doesNodeExist(0, input)) {
                // 类名不存在
          ...
          // attribute 名
            ...
            if (!pattern.test(input)) {
                // 格式不合法
                return "Valid Format: " + pattern.toString();
            } else if (model.doesNodeExist(2, input, stateOfPage.class)) {
                // attribute 名重复
          ...
          case "attribute-modify":
            ...
            if (!pattern.test(input)) {
                // 格式不合法
                return "Valid Format: " + pattern.toString();
            } else if (stateOfPage.attribute !== input && model.doesNodeExist(2, input, stateOfPage.class)) {
          ...
          case "type-modify":
            ...
            // build-in types
            if (!pattern.test(input) && !model.doesNodeExist(0, input)) {
                // 不是内置类型，也不是类
    ...
    // 检查单个输入框输入内容合法性
    function checkInput(model, $input) {
        // $input 是单个输入框组件
        ...
        if (undefined !== inputCase) {
            var input = $input.val();
            // 输入框内容
            var checkResult = getInputCheckResult(model, inputCase, input);
            // 合法性检查结果
    ...
    // 检查多个输入框输入内容合法性
    function checkInputs(model, $inputs) {
        // $inputs 是一组输入框组件
        var num = $inputs.size();
    ...
    function handleKbdCtrlInput(event) {
        if (13 === event.which) {
            ...
            if (0 !== $(this).parent().parent().find("#stigmod-search-left-btn, #searchButton").trigger("click").length) {
                // 第一个 parent() 是考虑了 typeahead wrapper 的影响
                return false;
            }
    ...
    function handleSubmitEdit(event) {
        ...
        if (checkInputs(icm, $visibleInputs)) {
            ...
            if ("title" === caseEdit) {
                ...
                var relationGroups = icm.getSubModel([ 1 ]);
                // 获取所有 relation group
                for (var nameRG in relationGroups) {
                    ...
                    if (relationGroups.hasOwnProperty(nameRG)) {
                        ...
                        if (null !== matchName) {
                            ...
                            for (var nameR in relations) {
                                // 遍历该 relation group 中的所有 relation
                                if (relations.hasOwnProperty(nameR)) {
                                    // 修改 relation 中的 class name
                                    var nameClass = relations[nameR][0]["class"];
                                    // 获取该 relation 两端的 class 的名字的引用
                ...
                for (var i = 0; i < num - 1; ++i) {
                    ...
                    if (0 === stateOfPage.flagCRG) {
                        ...
                        try {
                            ...
                            icm.modifyPropOfA(stateOfPage.class, stateOfPage.attribute, propertyName, newText);
                        } catch (error) {
                ...
                if (1 === stateOfPage.flagCRG) {
                    ...
                    try {
                        ...
                        icm.modifyPropOfR(stateOfPage.class, stateOfPage.attribute, propertyNameOfR, propertyValueOfR);
                    } catch (error) {
                        if (error instanceof ReferenceError) {
                            // 没有，增加
    ...
    function handleAddClassOk() {
        ...
        if (checkInput(icm, $input)) {
            ...
            // 关闭当前 modal
            $("#stigmod-nav-left-scroll").find("span[stigmod-nav-left-tag=" + className + "]").parent()[0].scrollIntoView();
            // 滚动使该元素显示在视口中
            enableSave();
        }
    }
    ...
    function handleAddRelGrpOk() {
        ...
        if (checkInputs(icm, $inputs) && isValidRelationGroup(icm, $(this).closest(".modal-footer"), relationGroupName)) {
            ...
            $(this).next().trigger("click");
            // 关闭当前 modal
            $("#stigmod-nav-left-scroll").find("span[stigmod-nav-left-tag=" + relationGroupName + "]").parent()[0].scrollIntoView();
            // 滚动使该元素显示在视口中
            enableSave();
        }
    }
    ...
    function handleMoveAttRelPanelDown() {
        ...
        if ($nextPanel.hasClass("panel")) {
            ...
            icm.moveOrderElem(stateOfPage.flagCRG, stateOfPage.class, name, 1);
            // 更新显示
            $nextPanel.after($thisPanel);
            // 下移当前 panel 节点
            enableSave();
        } else {}
    }
    // 处理：所有删除按钮的入口
    function handleRemoveEntrance() {
        // TODO：直接写图标类不是长久之计
        setTimeout(function() {
            // 延时是为了解决 stateOfPage 还没有更新 modal 就弹出的问题
    ...
    function handleRemoveOk() {
        ...
        switch (stateOfPage.flagDepth) {
          ...
          case 2:
            ...
            var strPanel = "#stigmod-cont-right .panel[stigmod-attrel-name=" + stateOfPage.attribute + "] " + prop[stateOfPage.flagCRG] + stateOfPage.property;
            var $root = $(strPanel);
    ...
    // 处理：modal 显示时复位
    function handleMdlRmTooltip() {
        // 对任何 modal 都有效
        $(this).find(".tooltip").remove();
    }
    function handleMdlFocus() {
        // 对任何 modal 都有效
        focusOnInputIn($(this));
    }
    ...
    function handleClkSave() {
        ...
        // 获取日志
        postData.orderChanges = icm.getAttRelOrderChanges();
        // 获取有变动的顺序数组
    ...
    function handleClkSearchLeft() {
        ...
        if (icm.doesNodeExist(0, name) || icm.doesNodeExist(1, name)) {
            // 如果模型中存在名为 name 的类或关系组
            $("#stigmod-nav-left-scroll").find("span[stigmod-nav-left-tag=" + name + "]").trigger("click").parent()[0].scrollIntoView();
        } else {}
        $(this).blur();
    }
    // 处理：点击进入全屏
