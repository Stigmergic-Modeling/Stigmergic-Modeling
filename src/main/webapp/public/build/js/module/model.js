define("js/module/model", [], function(require, exports, module) {
    ...
    function Model(modelPassIn) {
        ...
        // log
        this.operationLog = [];
        this.operationLogHistory = [];
        // 用于保存 log 的历史，便于回滚
        // 记录在“保存操作”区间内有哪些类的属性或哪些关系组中的关系发生了顺序变化，便于生成 AttRelOrderChanges。此记录便于后端更新数据。
        this.attRelOrderChanged = [ {}, {} ];
        // 第一个对象中保存 class 名，第二个对象中保存 relationGroup 名
        ...
        if (!(this.getSubModel instanceof Function)) {
            ...
            Model.prototype.removeSubModel = function(path, key) {
                ...
                if (1 === path.length) {
                    // 当删除类或关系组时
                    this.attRelOrderChanged[path[0]][key] = 0;
                }
                var subModel = this.getSubModel(path);
                delete subModel[key];
            };
            ...
            Model.prototype.insertOrderElem = function(flagCRG, classRelGrpName, attrRelName, position, direction) {
                ...
                this.attRelOrderChanged[flagCRG][classRelGrpName] = 1;
                // 表示该 classRelGrp 内的元素顺序有变
            ...
            Model.prototype.modifyOrderElem = function(flagCRG, classRelGrpName, attrRelName, newName) {
                ...
                // 表示该 classRelGrp 内的元素顺序有变
                var order = this.getSubModel([ flagCRG, classRelGrpName, 1, "order" ]);
                order.splice(order.indexOf(attrRelName), 1, newName);
            };
            // 在顺序数组中删除某元素
            Model.prototype.removeOrderElem = function(flagCRG, classRelGrpName, attrRelName) {
                ...
                // 表示该 classRelGrp 内的元素顺序有变
                var order = this.getSubModel([ flagCRG, classRelGrpName, 1, "order" ]);
                order.splice(order.indexOf(attrRelName), 1);
            };
            // 在顺序数组中某元素前移或后移
            Model.prototype.moveOrderElem = function(flagCRG, classRelGrpName, attrRelName, direction) {
                ...
                // 删除
                order.splice(index + direction, 0, attrRelName);
            };
            // 向控制台输出模型
            ...
            Model.prototype.clearLog = function() {
                this.operationLogHistory.push([ Date.now(), this.operationLog ]);
                // 保存 log 的历史，便于回滚
                this.operationLog = [];
            };
            // 检测日志是否为空
                    ...
                    if (attRel[1].hasOwnProperty(rlg)) {
                        if (1 === attRel[1][rlg]) {
                            ...
                            if (changes.relationGroups[rlg] !== void 0) {
                                delete changes.relationGroups[rlg];
                            }
                        }
                    }
                }
            ...
            Model.prototype.clearAttRelOrderChanges = function() {
                this.attRelOrderChangedHistory.push([ Date.now(), this.attRelOrderChanged ]);
                // 保存 顺序变动记录 的历史，便于回滚
                this.attRelOrderChanged = [ {}, {} ];
            };
            /*  ------- *
            ...
            Model.prototype.addAttr = function(className, attrName, pos) {
                ...
                this.addNode([ 0, className, 0 ], [ attrName, [ {} ] ]);
                // 空括号很重要
                this.insertOrderElem(0, className, attrName, pos.position, pos.direction);
            };
            // 增加类的属性的特性
            ...
            Model.prototype.addRelation = function(relGrpName, relID, pos) {
                ...
                this.addNode([ 1, relGrpName, 0 ], [ relID, [ {} ] ]);
                // 空括号很重要
                this.insertOrderElem(1, relGrpName, relID, pos.position, pos.direction);
            };
            // 增加关系的特性
            ...
            Model.prototype.modifyAttrName = function(className, oldName, newName) {
                this.modifyNodeName([ 0, className, 0 ], oldName, newName);
                // 修改 attribute 的 key
            ...
            Model.prototype.modifyRelID = function(relGrpName, oldID, newID) {
                this.modifyNodeName([ 1, relGrpName, 0 ], oldID, newID);
                // 修改 attribute 的 key
