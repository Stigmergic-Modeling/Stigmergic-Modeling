 ...
 */
@Controller
public class NoMergeDataController {
    @Autowired
    ...
    @RequestMapping(value="/unMerge", method = RequestMethod.GET)
    @ResponseBody
...
//            System.out.println("line: "+line);
            ...
            curSet.add(curUId);
            if(!valueListMap.containsKey("true")) {
                ValueNode trueVNode = new ValueNode();
                constructVNode(valueNodeList,trueVNode, valueListMap, modelId, new HashSet<Long>() , "true");
            }
            if(!line.equals("*******************")) {
...
//                        ValueNode mainRoleVNode = null;//该class类的role
                    ...
                    for(int i=3;i<strs.length;i++) {
                        ...
                        ValueNode classMultiVNode = null;
                        if(valueListMap.containsKey("1")) {
                            classMultiVNode = valueNodeList.get(valueListMap.get("1"));
                        }else {
                            classMultiVNode = new ValueNode();
                            constructVNode(valueNodeList,classMultiVNode,valueListMap,modelId,curSet,"1");
                        }

                        if(attrMulti.equals("")) attrMultiVNode = valueNodeList.get(valueListMap.get("1"));
                        else {
                            if(valueListMap.containsKey("*"))
                                attrMultiVNode = valueNodeList.get(valueListMap.get("*"));
                            else {
                                attrMultiVNode  = new ValueNode();
                                constructVNode(valueNodeList,attrMultiVNode,valueListMap,modelId,curSet,"*");
                            }
                        }
...
//                        System.out.println("multis: "+strs[3]);
                        ValueNode startMultiVNode = null;
                        ValueNode endMultiVNode = null;
                        if(valueListMap.containsKey(multis[0])) {
                            startMultiVNode = valueNodeList.get(valueListMap.get(multis[0]));
                        }else {
                            startMultiVNode = new ValueNode();
                            constructVNode(valueNodeList,startMultiVNode,valueListMap,modelId,curSet,multis[0]);
                        }
                        if(valueListMap.containsKey(multis[1])) {
                            endMultiVNode = valueNodeList.get(valueListMap.get(multis[1]));
                        }
                        else {
                            endMultiVNode = new ValueNode();
                            constructVNode(valueNodeList,endMultiVNode,valueListMap,modelId,curSet,multis[1]);
                        }
