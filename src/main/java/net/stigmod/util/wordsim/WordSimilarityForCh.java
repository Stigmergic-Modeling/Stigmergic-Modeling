 ...
 * @version 1.0
 */
public class WordSimilarityForCh {
    // 词库中所有的具体词，或者义原
    private static Map<String, List<WordForCh>> ALLWORDS = new HashMap<String, List<WordForCh>>();
    /**
    ...
    public static void loadGlossary() {
        ...
        try {
            ...
            String path = config.getHowNetGlossaryPath();
//            reader = new BufferedReader(new FileReader(path));
    ...
    /**
     * 解析具体概念部分，将解析的结果存入<code>WordForCh wordForCh</code>.
     *
     * @param related
     */
    public static void parseDetail(String related, WordForCh wordForCh) {
        // spilt by ","
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            if (type == 0) {
                ...
                // 之前有一个是符号义原
                if (isSimbol) {
                    wordForCh.addRelationSimbolPrimitive(simbolKey, chinese);
                    continue;
                }
                if (isFirst) {
                    wordForCh.setFirstPrimitive(chinese);
                    isFirst = false;
                    continue;
                } else {
                    wordForCh.addOtherPrimitive(chinese);
                    continue;
                }
            }
            // 关系符号表
            ...
            if (type == 2) {
                ...
                if (strs[0].startsWith("{")) {
                    ...
                    // 去掉有半部分 "}"
                    if (chinese != null) {
                        wordForCh.addStructruralWord(chinese);
                        continue;
                    } else {
                        // 如果没有中文部分，则使用英文词
                        wordForCh.addStructruralWord(english);
                        continue;
                    }
                }
            }
     ...
     * </p>
     * <p>
     * 0-----PrimitiveForCn<br/> 1-----Relational<br/> 2-----Special
     * </p>
     *
     * @param str
    ...
    public static double simWord(String word1, String word2) {
        if (ALLWORDS.containsKey(word1) && ALLWORDS.containsKey(word2)) {
            ...
            List<WordForCh> list2 = ALLWORDS.get(word2);
            double max = 0;
            for (WordForCh w1 : list1) {
                for (WordForCh w2 : list2) {
...
//        if(!ALLWORDS.containsKey(word2))System.out.println("单词: "+word2+" 没有被收录");
     ...
     * @return
     */
    public static double simWord(WordForCh w1, WordForCh w2) {
        // 虚词和实词的相似度为零
    ...
    private static double innerSimWord(String word1, String word2) {
        boolean isPrimitive1 = PrimitiveForCn.isPrimitive(word1);
        boolean isPrimitive2 = PrimitiveForCn.isPrimitive(word2);
        // 两个义原
    ...
    public static int disPrimitive(String primitive1, String primitive2) {
        List<Integer> list1 = PrimitiveForCn.getParents(primitive1);
        List<Integer> list2 = PrimitiveForCn.getParents(primitive2);
        for (int i = 0; i < list1.size(); i++) {
            int id1 = list1.get(i);
     ...
     * @param wordForCh
     */
    public static void addWord(WordForCh wordForCh) {
        List<WordForCh> list = ALLWORDS.get(wordForCh.getWord());

        if (list == null) {
            list = new ArrayList<WordForCh>();
            list.add(wordForCh);
            ALLWORDS.put(wordForCh.getWord(), list);
        } else {
            list.add(wordForCh);
        }
    }
